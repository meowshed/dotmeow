# components/macos-defaults/init.star
#
# platform: macos
#
# Applies macOS system defaults via defaults(1), pmset, nvram, and
# related tools. Idempotent — safe to re-run.

after = ["@stdlib//bundles/modern-macos"]

def install(ctx):
    os = ctx.run(["uname", "-s"]).strip()
    if os != "Darwin":
